...
namespace LibraryChallengeWeb.Controllers.Api
{
    public class LibraryBooksController : ApiController
    {
        ...
        [Route("api/library/sortedbooks")]
        public HttpResponseMessage GetAllLibraryBooksSorted()
        {
            var libraryService = new LibraryService();

           // IList<ILibraryBook> books = libraryService.AllBooks();
            IEnumerable<ILibraryBook> _books = libraryService.AllBooksCategorized();
            return Request.CreateResponse(HttpStatusCode.OK, _books);
        }

        [Route("api/library/categorizeditems")]
        public HttpResponseMessage GetAllBooksInCategory()
        {
            var libraryService = new LibraryService();
            IList<LibraryChallengeCore.IBookCategory> _books = libraryService.AllBooksByCategory();
